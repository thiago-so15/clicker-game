...
class DevPanel {
    ...
    makeDraggable(element) {
        ...
        header.addEventListener('mousedown', (e) => {
            if (e.target.classList.contains('dev-panel-close'))
                return;
            isDragging = true;
            startX = e.clientX;
            startY = e.clientY;
    ...
    completeMissions() {
        ...
        const missions = this.gameInstance['missions'];
        let completed = 0;
    ...
    updateStateDisplay() {
        ...
        if (scoreEl)
            scoreEl.textContent = this.formatNumber(gameState.get('game.score') ?? 0);
        if (clickEl)
            clickEl.textContent = String(gameState.get('game.clickUpgradeLevel') ?? 0);
        if (autoEl)
            autoEl.textContent = String(gameState.get('game.autoUpgradeLevel') ?? 0);
        if (prestigeEl)
            prestigeEl.textContent = String(gameState.get('prestige.level') ?? 0);
    }
    /**
     * Formatear número
     */
...
class MicroAnimations {
    constructor() {
        ...
        // Configuración
        this.COMBO_THRESHOLD_MS = 300; // ms entre clicks para combo
        this.COMBO_DECAY_MS = 1000; // ms para perder el combo
        this.init();
    }
    static getInstance() {
    ...
    updateComboIndicator() {
        if (!this.comboIndicator || !this.clickButton)
            return;
        if (this.comboCount >= 3) {
     ...
     */
    pulseButton() {
        if (!this.clickButton || !this.isEnabled())
            return;
        // Efecto ripple
     ...
     */
    animateValueUpdate(element, className = 'updated') {
        if (!this.isEnabled())
            return;
...
class PlayerAnalyzer {
    constructor() {
        ...
        this.closeBtn = null;
        // Estado interno
        this.clickHistory = []; // Timestamps de clicks recientes
        ...
        this.gameInstance = null;
        // Configuración
        this.CPS_WINDOW_MS = 5000; // Ventana para calcular CPS
        this.MIN_TIP_INTERVAL_MS = 30000; // Mínimo entre tips
        this.ANALYSIS_INTERVAL_MS = 10000; // Cada cuánto analizar
     ...
     */
    showNextTip() {
        if (this.isShowing || this.tipQueue.length === 0)
            return;
        const now = Date.now();
        if (now - this.lastTipTime < this.MIN_TIP_INTERVAL_MS)
            return;
     ...
     */
    calculateUpgradePrice(level, base, multiplier) {
...
class ClickerGame {
    ...
    handleClick(event) {
        ...
        if (event) {
            ...
            microAnimations.handleClick(buttonRect.left + buttonRect.width / 2, buttonRect.top + buttonRect.height / 2, points);
        }
